 ...
 */
public class Clients extends SimProcess {
	...
	*/
	 ...
	 */
	public Clients(Model owner, String name, boolean showInTrace) {
	...
	public void lifeCycle() throws SuspendExecution {
		...
		myModel.clientsQueue.insert(this);
		startWait = presentTime();
		sendTraceNote("ClientQueuelength: " + myModel.clientsQueue.length());

		// is the VC available ?
		if (!myModel.idleEmployeeQueue.isEmpty()) { // it is available

			//get the first (and only) VC from the idle VC queue
			Employee employee = myModel.idleEmployeeQueue.first();
			//to get it does not mean it is removed yet (?!), so do it now:
			myModel.idleEmployeeQueue.remove(employee);
		...
		// Bye!
                //Todo: goncer: Add into the queue to pay!
		sendTraceNote("Client was serviced and leaves system.");
                sendTraceNote("timeWasted :" + myModel.totalWastedTime);
		myModel.clientsServiced.update(++myModel.servicedClients);
